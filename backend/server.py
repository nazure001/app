...
from google.genai.types import HarmBlockThreshold, PersonGeneration
...
def generate_gemini_concept(data: BuildRequest):
    ...
    {{
        ...
        "image_prompt": (
            f"A cinematic shot of minecraft {data.style} {data.idea} in "
            f"{data.biome}, rtx on, 8k, photorealistic textures, dramatic lighting"
        )
    }}
    """
...
async def generate_images_with_gemini(base_prompt: str, request: BuildRequest):
    ...
    try:
        ...
        view_prompts = {
            "cinematic": (
                f"{base_prompt}, cinematic angle, dramatic golden hour lighting, "
                "detailed minecraft blocks, photorealistic render, epic atmosphere, "
                "high quality, 8k resolution"
            ),
            "palette": (
                f"{base_prompt}, block palette showcase, material breakdown, "
                "organized grid layout, clean white background, "
                "inventory view style, all blocks visible, 8k"
            ),
            "angle": (
                f"{base_prompt}, isometric 3D view, technical rendering, "
                "clear block details, schematic style, white background, "
                "architectural drawing, 8k resolution"
            ),
            "blueprint": (
                f"{base_prompt}, blueprint architectural drawing style, "
                "technical schematic, top-down view, blue lines on white, "
                "grid overlay, construction details, 8k"
            ),
        }
            ...
            try:
                response = genai_client.models.generate_images(
                    ...
                    config=types.GenerateImagesConfig(
                        number_of_images=1,
                        safety_filter_level=HarmBlockThreshold.BLOCK_LOW_AND_ABOVE,
                ...
                if response.generated_images and len(response.generated_images) > 0:
                    image_url = response.generated_images[0].gcs_uri  # type: ignore [attr-defined]
                    if image_url:
                        images_dict[view_type] = image_url
                        print(f"✓ Generated {view_type} image via Gemini")
                    else:
                        print(
                            "✗ No URL in response for "
                            f"{view_type}, using Pollinations fallback"
                        )
                        images_dict[view_type] = generate_safe_image_url(
                            enhanced_prompt
                        )
                else:
                    print(
                        "✗ No images in response for "
                        f"{view_type}, using Pollinations fallback"
                    )
                ...
                print(
                    f"✗ Error generating {view_type}: {e}, "
...
async def generate_build(request: BuildRequest):
    ...
    prompt_for_image = concept_data.get(
        "image_prompt",
        f"{request.style} {request.idea} minecraft build",
    )

    # Try to use Google Gemini image generation first (higher quality)
    images_dict = {}
    if genai_client:
        images_dict = await generate_images_with_gemini(prompt_for_image, request)

    # Fallback to Pollinations if Gemini unavailable or failed
    if not images_dict:
        images_dict = {
            "cinematic": generate_safe_image_url(
                prompt_for_image
                + (
                    ", cinematic view, detailed lighting, 8k render, "
                    "photorealistic, voxel art"
                )
            ),
            "palette": generate_safe_image_url(
                prompt_for_image
                + (
                    ", flat lay block palette style, 8k, organized grid, "
                    "white background"
                )
            ),
            "angle": generate_safe_image_url(
                prompt_for_image
                + (
                    ", Isometric technical drawing, 8k, schematic style, "
                    "white background"
                )
            ),
            "blueprint": generate_safe_image_url(
                prompt_for_image
                + (
                    ", blueprint schematic style, 8k, blue background, "
                    "technical drawing"
                )
            ),
        }
    ...
    return {**concept_data, "images": images_dict}
